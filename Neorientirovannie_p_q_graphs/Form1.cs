...
namespace Pz19_1
{
    public partial class Form1 : Form
    {
        ...
        static public void PrintArray(int[,] arr, char[] A, int n, int m, StreamWriter sw, int count)//Вывод массива
        {
            sw.Write("Номер - " + count);
            sw.WriteLine();
            for (int i = 0; i < n; i++)
                sw.Write(A[i] + " ");
            sw.WriteLine();
        ...
        private void Calculation_Click(object sender, EventArgs e)
        {
            ...
            int q = (tb_q.Text != "") ? Convert.ToInt32(tb_q.Text) : 0;
            int k = 2;
            int[] Soch2 = new int[k];
            ...
            if (q < m)
            {
                ...
                count_pq++;
                PrintArray(Matrix, A, p, p, sw, count_pq);
                Reset(ref Matrix, p, p);
                ...
                else
                {
                    ...
                    while (NextCombobj(Soch_q, m, q))
                    {
                        ...
                        PrintArray(Matrix, A, p, p, sw, count_pq + 1); //выводим в файл
            ...
            else rtb.Text += "\nЧисло заданных ребер больше допустимого числа";
        }
    }
}
